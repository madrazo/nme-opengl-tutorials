...
public function indexVBO (
    ...
    var mDebugTextField:TextField;
    public inline function addDebugText ()
    {
        mDebugTextField = new TextField();
        addChild(mDebugTextField);
        mDebugTextField.autoSize = TextFieldAutoSize.LEFT;
        mDebugTextField.background = true;
        mDebugTextField.defaultTextFormat.size = 200;
        mDebugTextField.multiline = true;
        if (GL3Utils.isGLES3compat())
        {
            trace("Compatible with GLES3 API");
            mDebugTextField.text = "Compatible with GLES3 API"+String.fromCharCode(10);
        }
        else
        {
            trace("Not compatible with GLES3 API");
            mDebugTextField.text = "Not compatible with GLES3 API or forced GLES2"+String.fromCharCode(10);
        }
        ...
        mDebugTextField.text += "GL_NUM_EXTENSIONS:"+numExtensions+String.fromCharCode(10);
        //print list
        var i:Int = 0;
        ...
        if(Lambda.has(extensionList,"GLEW_AMD_seamless_cubemap_per_texture"))
        {
            ...
            mDebugTextField.text += "The GL_AMD_seamless_cubemap_per_texture is present, (but we're not goint to use it)"+String.fromCharCode(10);
        ...
        {
            mDebugTextField.text += "The GL_AMD_seamless_cubemap_per_texture is not present (maybe you are on NV)"+String.fromCharCode(10);            
        }
        ...
        if(Lambda.has(extensionList,"GL_KHR_debug"))
        {
            ...
            mDebugTextField.text += "The OpenGL implementation provides debug output. Let's use it !"+String.fromCharCode(10);
            //needs NME mod
            GL.debugMessageCallback(onError);
        }
        else
        {
            mDebugTextField.text += "GL_KHR_debug unavailable. You have to use glGetError() and/or gDebugger to catch mistakes."+String.fromCharCode(10);            
        }
    }
	...
	#if test
        if(mDebugTextField.text.length < 2048)
            mDebugTextField.text += "Got a GL ERROR : "+message+String.fromCharCode(10);
	#end
        //throw ("Got a GL ERROR : "+message);
        do{}while(true);
    }
...
class PackedVert
{
    ...
    public function hashCode():Int
    {
        ...
        var key:haxe.Int64 = haxe.Int64.make( xi, yi);
